...
RobotUtils = (function(self) {
     ...
     *
     */
    self.waitForServices = function(serviceList) 
    {
        var i;
        for ( i = 0; i < arguments.length; i++ ) {
            if ( $.inArray(arguments[i], self.waitedServices) == -1 ) {
                self.waitedServices.push(arguments[i]);
            }
        }
    }
    ...
    self.onServices = function(servicesCallback, errorCallback) {
        self.connect(function(session) {
            ...
            for (i = 0; i < wantedServices.length; i++) {
                (function (i){
                    getService(session, i, wantedServices[i], function(i, service) {
                        services[i] = service;
                        pendingServices -= 1;
     ...
     *
     */
    function getService(session, index, serviceName, onSuccess, onFailure) {
        session.service(serviceName).then(
            function(service) {
                onSuccess(index, service);
            },
            function() {
                if ( $.inArray(serviceName, self.waitedServices) != -1 ) {
                    console.log("Waiting for service "+serviceName);
                    setTimeout(function(){ getService(session, index, serviceName, onSuccess, onFailure) }, 1000);
                }
                else {
                    onFailure(service);
                }
